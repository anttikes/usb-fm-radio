...
__ALIGN_BEGIN static uint8_t USBD_CUSTOM_HID_CfgDesc[USB_CUSTOM_HID_CONFIG_DESC_SIZ] __ALIGN_END = {
	...
	HIBYTE(USB_CUSTOM_HID_CONFIG_DESC_SIZ), 0x01, /* bNumInterfaces: 1 interface */
...
static uint8_t USBD_CUSTOM_HID_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
	...
	hhid = (USBD_CUSTOM_HID_HandleTypeDef*) USBD_malloc (
			sizeof(USBD_CUSTOM_HID_HandleTypeDef));
...
{
	USBD_CUSTOM_HID_HandleTypeDef *hhid =
			(USBD_CUSTOM_HID_HandleTypeDef*) pdev->pClassDataCmsit[pdev->classId];
	uint16_t len = 0U;
	...
	case USB_REQ_TYPE_STANDARD:
		...
		case USB_REQ_GET_DESCRIPTOR:
			if ((req->wValue >> 8) == CUSTOM_HID_REPORT_DESC)
			{
				...
				pbuf =
						((USBD_CUSTOM_HID_ItfTypeDef*) pdev->pUserData[pdev->classId])->pReport;
			}
			else
			{
...
static uint8_t* USBD_CUSTOM_HID_GetFSCfgDesc(uint16_t *length)
{
	...
	USBD_EpDescTypeDef *pEpOutDesc = USBD_GetEpDesc(USBD_CUSTOM_HID_CfgDesc,
...
static uint8_t USBD_CUSTOM_HID_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
	...
	((USBD_CUSTOM_HID_HandleTypeDef*) pdev->pClassDataCmsit[pdev->classId])->state =
			CUSTOM_HID_IDLE;
