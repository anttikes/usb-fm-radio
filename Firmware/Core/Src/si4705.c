...
#define CHIP_MAX_SETTING  63        // Target chip register max (Loudest: 0 dB)
#define CHIP_MIN_SETTING  0         // Target chip register min (Quietest: -63 dB)
...
uint8_t map_usb_to_chip_volume(int16_t usb_vol_value)
{
	...
	//    Formula: chip_setting = CHIP_MAX_SETTING + db_attenuation
	//    Example: 0 dB -> 63 + 0 = 63
	//    Example: -63 dB -> 63 + (-63) = 0
	int16_t chip_setting = CHIP_MAX_SETTING + db_attenuation;
...
int16_t map_chip_to_usb_volume(uint8_t chip_setting)
{
	...
	int16_t db_attenuation = (int16_t)chip_setting - CHIP_MAX_SETTING;
